...
- **D-032 / B-010: `MpvBridgeModule` extends `NativeMpvPlayerSpec`
  generated by `1.5.5`, completing the new-arch TurboModule
  wire-up that 1.5.5 shipped only partially.** This is the **proper
  fix** for the "Player refused launch" toast — the in-session patches
  from 1.5.5 alone were insufficient.

  Root cause recap (from 1.5.4 / 1.5.5): RN 0.86 new-arch requires
  every TurboModule Kotlin class to extend the codegen-generated
  `<lib>Spec` class (or to otherwise be discoverable by the new-arch
  TurboModuleManager). The TS spec at `src/bridge/NativeMpvPlayer.ts`
  triggers codegen to emit
  `com.facebook.fbreact.specs.NativeMpvPlayerSpec` — but that only
  helps if `MpvBridgeModule` actually *extends* it. Pre-1.5.6, the
  Kotlin class still extended `ReactContextBaseJavaModule` directly.
  Result: the `NativeMpvPlayerSpec.java` was created on disk but
  never instantiated; `TurboModuleManager` couldn't find a Kotlin
  class implementing it; `NativeModules.MpvPlayerModule` resolved to
  `undefined`; `resolveBridge()` in `MpvPlayerModule.ts:624-637`
  returned `null`; `openPlayer()` fell through to
  `NOOP_BRIDGE.openPlayer` which resolves `false`; the Movies / Search
  / Music / AllVideos player toast showed the misleading "Player
  refused launch" string. No logcat warning, no compile error.

  Why 1.5.5 alone was insufficient:
    1. `apply plugin: "com.facebook.react"` was restored in
       `android/build.gradle` (✓ 1.5.5) — without this the consumer's
       codegen pipeline would have run with no module at all.
    2. `src/bridge/NativeMpvPlayer.ts` was added as the
       `extends TurboModule` source the combine-cli scans for (✓ 1.5.5).
       Codegen then wrote `NativeMpvPlayerSpec.java` containing 70
       abstract methods (plus one concrete `getName()`).
    3. **MISSING in 1.5.5:** `MpvBridgeModule.kt` still extended
       `ReactContextBaseJavaModule` — **not** `NativeMpvPlayerSpec`.
       The class was technically compilable, but it was not a
       `TurboModule` subclass from the codegen runtime's perspective,
       so the new-arch registry silently refused to wire it up.

  Detection recipe (still no logcat warning):
    - `android/app/build/generated/source/codegen/java/com/facebook/fbreact/specs/NativeMpvPlayerSpec.java` is present (codegen ran)
    - `javap -classpath app/build/intermediates/library_android_classes_debug/classes.jar -p com.simba.player.mpv.MpvBridgeModule` shows `extends com.facebook.react.bridge.ReactContextBaseJavaModule` (the bug)
    - `dumpsys package com.simba.player` lists no native methods on `MpvPlayerModule` (RN 0.86+ bridgeless)

  Fix:
  - **`android/src/main/java/com/simba/player/mpv/MpvBridgeModule.kt`** —
    class now `extends NativeMpvPlayerSpec(reactContext), IMpvConfigProvider,
    IMpvNativePtrProvider, IPipModeChangeEmitter`. Every existing
    `@ReactMethod` declaration is now also marked `@Override` (Kotlin
    annotation, redundant with the modifier but the convention used
    elsewhere in the codebase) and uses the `override fun` modifier,
    so the Kotlin compiler enforces the spec invariant at build time.
  - **Signature normalizations** (`Int` → `Double`, `String?` →
    `String`) applied to satisfy the codegen's `double` /
    non-null-`String` mapping:
    - `dumpObservedProperties(): Int` → `Double` (spec returns `double`)
    - `openPlayer(... title: String?, ...)` → `title: String`
      (spec is non-null; the Kotlin side already null-checked
      upstream)
    - `enterPip(chapterTitle: String? = null, progressPct: String? = null)`
      → `enterPip(chapterTitle: String, progressPct: String)` (no
      defaults; defaults are dropped because there are no internal
      Kotlin callers — JS is the only entry point)
    - Body fix inside `openPlayer`:
      `val resolvedTitle = title?.takeIf { it.isNotBlank() } ?: uri`
      → `val resolvedTitle = title.takeIf { it.isNotBlank() } ?: uri`
  - **Async-ified `getLaunchParams()`**: the V12 synchronous
    `isBlockingSynchronousMethod = true` version returned
    `WritableMap?`. The codegen spec declares it as
    `void getLaunchParams(Promise promise)`. The implementation now
    matches the spec: when `lastLaunchParams == null` we
    `promise.resolve(null)`; otherwise we clear the one-shot, build a
    `WritableMap`, and `promise.resolve(map)`. JS callers should
    `await bridge.getLaunchParams()` — note the consumer's
    `useLaunchParams.tsx` still calls it synchronously (a TS-side
    follow-up awaited by the W22 in-app play audit; out of scope for
    this patch).
  - **Added missing `toggleMute()`**: the spec's 71st abstract method
    was not previously implemented in Kotlin (callers went through
    `setMuted(!getMuted())` instead). 1.5.6 adds it as
    `toggleMute() = nativeSetMuted(nativePtr, !nativeGetMuted(nativePtr))`.
  - **`getName()` already overrides the spec** — the spec provides a
    concrete `public @Nonnull String getName() { return NAME; }` and
    the Kotlin class's existing `override fun getName(): String = NAME`
    satisfies it (no change needed).

### Why the consumer's gradle build verifies the fix

The codegen task `:simba-dev_react-native-media-player:generateCodegenArtifactsFromSchema`
only runs successfully when:
  (a) `src/bridge/NativeMpvPlayer.ts` exists with `extends TurboModule`,
      and
  (b) the resulting spec matches the Kotlin module's parent class.

Concretely, after 1.5.6:
  - `gradlew :simba-dev_react-native-media-player:compileDebugKotlin` succeeds with
    `BUILD SUCCESSFUL` (the Kotlin compiler enforces the override
    contract — any missed abstract method produces
    "Class 'MpvBridgeModule' is not abstract and does not override
    abstract member '...'"; any signature mismatch produces
    "override fun X(...): nothing was overridden"; etc.).
  - The compiled AAR's `MpvBridgeModule.class` shows
    `Superclass : 'Lcom/facebook/fbreact/specs/NativeMpvPlayerSpec;'`
    in `dexdump` (instead of
    `Lcom/facebook/react/bridge/ReactContextBaseJavaModule;`).
  - At runtime, `TurboModuleRegistry.getEnforcing<Spec>('MpvPlayerModule')`
    resolves to the live `MpvBridgeModule` instance, the
    `resolveBridge()` check at `MpvPlayerModule.ts:624-637` succeeds,
    and `openPlayer()` actually launches `PlayerActivity`.

### Migration from 1.5.5

None on the source level — `MpvBridgeModule`'s public Kotlin API
surface (`@ReactMethod`-exposed methods, IMpvConfigProvider /
IMpvNativePtrProvider / IPipModeChangeEmitter interface contracts,
PlayerPackage registration, `@ReactModule(name = ...)`) is byte-
identical at the consumer's reflection boundary. The fix is purely
the parent class declaration.

Runtime migration (consumer side, pre-1.5.6 follow-up tracked
separately under B-010 / W22 device task drive):
  - `bridge.getLaunchParams()` is now async (Promise) on the JS
    side even though `MpvPlayerModuleBridge.getLaunchParams()` in
    the lib still types it as sync (`LaunchParams | null`).
    `useLaunchParams.tsx` should switch to `await bridge.getLaunchParams()`
    on next pass. Out of scope here; tests still pass because they
    mock `getLaunchParams: jest.fn().mockReturnValue(null)` which is
    sync.

### Notes

- 1.5.5's CHANGELOG entry was effectively a re-publication of 1.5.4
  under a new version (npm auth token rotation). 1.5.5's published
  artifact is the same code bytes as 1.5.4.
- 1.5.6 is the **first** release where the codegen spec wire-up
  actually closes — meaning `gradlew :app:assembleDebug` on the
  consumer app now boots without `NativeModules.MpvPlayerModule is
  undefined` in logcat.
- Future TurboModule specs for this lib will follow the same
  pattern: Kotlin class extends the generated `<Module>Spec` and
  every `@ReactMethod` is marked `@Override override fun ...(...)`.
- Verified on SIMBA V22 (consumer app, RN 0.86, newArchEnabled=true,
  ...
  `PlayerActivity` via the live bridge (mpv surface + media controls
  visible). No logcat warning. Confirmed `dexdump` shows
  `MpvBridgeModule`'s superclass is `NativeMpvPlayerSpec`.
